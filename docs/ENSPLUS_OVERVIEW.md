...
- ✅ **Sentinel Lock** — opt-in transfer + unwrap timelocks, M-of-N guardian co-signs, instant panic freeze: a wallet drainer gets *nothing* during the delay. Honest wording (per our claims register): "theft requires defeating your timelock, your guardians, and your alerts."
...
**Built and verified (146/146 tests, cross-fuzz EVM ≡ JS mirror, solc 0.8.26, zero warnings) — 22 contracts across 14 slices:** the pure libraries (category, attestation, weight, trust); ENSPLUSVault + RevenueSplitter with invariants I1–I4; the complete InternalGovernor; the constitutional machinery (GovernorExecuted, ConstitutionRegistry, ModuleRegistry with on-chain machine checks, StandingOrders); AttestorRegistry, GovernorAdapter (directional nominal casting — the live ENS governor has no fractional counting), VaultSteward; NameVault (dual custody, D7 controller retention, per-owner index, migration + sentinel slots); RenewalPool (first real charter); SentinelLock (theft timelocks/guardians/panic, I10); Watchtower (expiry escalation, alarms, resurrection anchor); Citizen + ParticipationCredits (soulbound + ERC-6551, charter-gated credits); LibTrust + TrustOracle (L1-native sybil-resistant reputation, live against every registry); HumanAttestor (ownerless zkPassport proof-of-humanity); CitizenResolver (ENS resolver for `ensplus.*` civic records, recordVersion + ENSIP-10 + CCIP). Plus the ensplus-utils SDK, on-chain EFP reader, EIK profile resolver/renderer, gas snapshot, and deterministic deploy blueprint.

**Deferred to later, separately-audited waves (asset-moving / higher-risk):** marketplace/leasing/foundry, inheritance/dead-man's-switch, curated-category TCR/guilds/bounties, the Specimen Plate art system, Protocol Watch, and the ENSv2 migration kit (M1–M5, blocked on v2 going live). Audit scope is a deliberately tight core.

**Gating genesis (deliberately):** the derivation dry-run (freezes era/rank semantics against three corpora; archive the Grails corpus before its sunset); the constitutional text in ratifiable language; the policy-accounting decision for per-holder silent-weight; parameter review (cap vs community size — a real finding from our own tests); legal review (utility-first); pre-audit hardening (invariant tests on custody-adjacent contracts); and **external audit(s) scoped to invariants I1–I10 — the top item, and the one thing that makes ENSPLUS as *proven* as it is *reasoned*.** (Resolved already: live-governor verification — no fractional, adapter pivoted to directional; EFP dependency — read on-chain + trust rebuilt L1-native.) We ship when the audits say so — not when a roadmap does, and not when ENS Labs' schedule does.
